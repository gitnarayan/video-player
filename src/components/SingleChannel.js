...
const SingleChannel = () => {
  ...
  const getSingleChannelbyId = () => {
    let channelID = localStorage.getItem("singlechannel_id");

    let SingleChannelbyId = {      
      _id: channelID
    };


  axios
  .post("http://103.104.74.215:3003/api/get_single_channel",SingleChannelbyId) 
  .then((response) => {
  ...
  return (
    <div className="single-channel-page" id="content-wrapper">
      <div className="single-channel-image">
        <img className="img-fluid" alt src={`http://103.104.74.215:3003/uploads/${channeldata?.image[0]?.filename}`}   style={{height:"15rem"}}/>

       
        <div className="channel-profile">
          <img className="channel-profile-img" alt    src={`http://103.104.74.215:3003/uploads/${channeldata?.image[1]?.filename}`} />
